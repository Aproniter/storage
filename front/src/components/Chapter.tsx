...
import { IChapter, IProject, INote, IImage} from '../models'
...
export function Chapter({ chapter, project }: ChapterProps) {
    ...
    async function getChapterFile(project_id: number, chapter_id: number, filename: string) {
        return axios.get(BaseURL+'/get_file/'+project_id+'/'+chapter_id+'/', {
            responseType: 'blob',
        })        
    ...
    async function fetchPreview(project_id: number, chapter_id: number) {
        setDownloading(true)
        if(images.length === 0){
            const responce = await axios.get<IImage[]>(BaseURL+'/get_preview/'+project_id+'/'+chapter_id+'/')
        ...
        >
            ...
            <div className='tools flex'>
                <div className="tool p-1 hover:shadow shadow-2xl" data-tooltip='Предпросмотр'>
                    <svg
                        className='w-8 h-8'
                        onClick={() => fetchPreview(project.id, chapter.id,)}
                ...
                <div className="tool p-1 hover:shadow shadow-2xl" data-tooltip='Скачать документ'>
                    <svg 
                        className='w-8 h-8'
                        onClick={() => getChapterFile(project.id, chapter.id, chapter.title)}
